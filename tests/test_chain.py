...
def test_chain_never_changes_length():
    ...
    audio = speech_like()
    out, info = chain.process(audio, RATE, AudioSettings(declick=True),
                              0.0, True, -20.0, None)
...
def test_chain_hits_the_loudness_target():
    ...
    out, _ = chain.process(speech_like(20.0), RATE, AudioSettings(),
                           0.0, True, -20.0, None)
...
def test_output_stays_under_the_ceiling():
    ...
    out, _ = chain.process(speech_like(level=0.5), RATE, AudioSettings(),
                           0.0, True, -14.0, None)
...
def _with_transient(freq, amp=0.4, at_s=3.0, seconds=6.0):
    ...
    audio[0, at:at + burst.size] += (amp * np.sin(2 * np.pi * freq * burst)
                                     ).astype(np.float32)
...
def test_declick_would_be_dead_with_a_maximum_reference():
    ...
    """
    click, window = _with_transient(9000)
    assert not np.allclose(chain.declick(click, RATE)[0, window],
                           click[0, window], atol=1e-6)
...
def test_a_plugin_that_changes_length_is_refused():
    ...
    with pytest.raises(chain.ChainError, match="pituutta"):
        chain.process(speech_like(), RATE, AudioSettings(), 0.0, True, -20.0,
                      Truncating())
...
def test_room_tone_is_not_compressed():
    ...
    out, _ = chain.process(audio, RATE, AudioSettings(high_pass_hz=0),
                           0.0, False, None, None)
